def merge(A, B, low, mid, high): # B to tablica pmocnicza
    ...
    if i <= mid:
        while i <= mid:
            ...
            k += 1
            i += 1
    if j <= high:  #na dobrą sprawę ten elif jest nie potrzebeny jeżeli w talbicy B mamy skopiowaną A, ale
        while j <= high: # raczej nie będziemy mieli bo to się nie opłaca, B pewnie będzie wypełniona zerami
            B[k] = A[j]
            k += 1
            j += 1

...
def decreasing_edges(E, source, finish):
    max_vertex = 0
    ...
    for i in range(len(possible_source)):
        DFS_visit(edges, possible_source[i], finish, visited, result)
        if result[0]:
            return True
    return False

def DFS_visit(graph, source, finish, visited, result):
    ...
    for i in range(len(graph)):
        if graph[i][0] == source[1] and source[2] > graph[i][2] and not visited[i]:
            ...
            visited[idx] = True
            DFS_visit(graph, graph[i], finish, visited, result)
